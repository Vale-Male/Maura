            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        if (auth()->user()->can('product.view')) {
                            $sub->url(
                                action('SuplierController@index'),
                                __('lang_v1.list_supliers'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'products' && request()->segment(2) == '']
                            );
                        }
                        if (auth()->user()->can('product.create')) {
                            $sub->url(
                                action('SuplierController@create'),
                                __('product.add_suplier'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'products' && request()->segment(2) == 'create']
                            );
                        }
                    },
                    ['icon' => 'fa fas fa-user', 'id' => 'tour_step5']
                )->order(20);
            }
                    ...
                    function ($sub) {
                        if (auth()->user()->can('user.view')) {
                            $sub->url(
                                ...
                                __('lang_v1.list_employees'),
                                ['icon' => 'fa fas fa-user', 'active' => request()->segment(2) == 'users']
                            );
                        }
                        if (auth()->user()->can('user.create')) {
                            $sub->url(
                                action('ManageUserController@create'),
                                __('product.add_employee'),
